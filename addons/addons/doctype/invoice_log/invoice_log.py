...
class InvoiceLog(Document):
	def after_insert(self):
		...
		if self.event_owner_id:
			...
			new_doc = frappe.get_doc("Sales Invoice")
			...
			new_doc.append("items",{
				...
				"qty": 1,
				"rate": frappe.utils.flt(total_commission),
				"amount": frappe.utils.flt(total_commission),
...
def create_invoice_log_debug():
	self = frappe.get_doc("Invoice Log","105008")
	# create Sales Invoice
	if self.event_owner_id:
		try:
			customer_doc = frappe.get_doc("Customer",{"event_owner_id":self.event_owner_id})
